...
export const ensureOrder = (options) => {
  ...
  docs.forEach((doc) => {
    const key = cacheKeyFn ? cacheKeyFn(doc[prop]) : doc[prop];
    return docsMap.set(key, doc);
  });
  // Loop through the keys and for each one retrieve proper document. For not
  // existing documents generate an error.
  return keys.map((key) => {
    const normalizedKey = cacheKeyFn ? cacheKeyFn(key) : key;
    return (
      docsMap.get(normalizedKey) ||
  ...
  protected createLoader(
    ...
    return new DataLoader<ID, Type>(async (keys) => {
      return ensureOrder({
        ...
        keys,
        cacheKeyFn: options.dataloaderConfig?.cacheKeyFn,
